...
#    for CHN in ['Wen','Wmn','Zee','Zmm','Znn']:
        for i in range(0,len(CHN_DICT_SR[CHN])):
            ...
            os.system(('./scripts/postFitPlot_vhcc.py' \
                       ' --file=shapes_v7_jes_240820_preOnly.root --ratio --extra_pad=0.53 --file_dir=%(OUTNAME)s' \
                       ' --ratio_range 0.0,2.0 --empty_bin_error --channel=%(CHN)s --blind --x_blind_min 0.75 --x_blind_max 1.0 --x_title "BDT output" --y_title "Events" --mu ", #mu=1"' \
...
#              EXTRALABEL = CHN_DICT_SR[CHN][i][2]
#              os.system(('./scripts/postFitPlot_vhcc.py' \
#                  ' --file=shapes_v7_kinFit_240820.root --ratio --extra_pad=0.53 --file_dir=%(OUTNAME)s' \
#                  ' --ratio_range 0.4,1.6 --empty_bin_error --channel=%(CHN)s --blind --x_blind_min 0.75 --x_blind_max 1.0 --x_title BDT' \
