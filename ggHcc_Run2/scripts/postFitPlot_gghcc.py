...
parser.add_argument('--doZ',    default=False, help='Set True for the Z analysis')
...
plot_background_schemes = {'inclusive':[]}
...
if args.subbkg!=0:
  ...
  vvhist = getHistogram(histo_file,'zcc',file_dir, mode, logx=log_x)[0]
...
if not args.no_signal:
  ...
  if args.subbkg==0:
    if not args.doZ:
      legend.AddEntry(sighist,"ggH(H#rightarrowc#bar{c})x100","l")
    if args.doZ:
      legend.AddEntry(sighist,"Z#rightarrowc#bar{c}x100","l")
  else:
    legend.AddEntry(sighist,"ggH(H#rightarrowc#bar{c})","f")
  if args.subbkg!=0:
    legend.AddEntry(vvhist,"Z#rightarrowc#bar{c}","f")
else:
